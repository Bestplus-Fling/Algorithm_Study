import sys
sys.stdin = open('input.txt', 'r')
input = sys.stdin.readline


def bfs():
    global N, M, K, grid
    dxy = (1, 0), (0, 1), (-1, 0), (0, -1)
    visited = [[[False] * M for _ in range(N)] for __ in range(K+1)]
    queue = [(0, 0, K, 1, 1)]
    visited[K][0][0] = True
    while queue:
        temp = []
        for x, y, z, cnt, day_night in queue:
            if x == N-1 and y == M-1:
                return cnt
            cnt += 1
            day_night = 1 - day_night
            for dx, dy in dxy:
                nx, ny = x + dx, y + dy
                if nx < 0 or nx >= N or ny < 0 or ny >= M:
                    continue
                if grid[nx][ny] == 1 and z > 0 and not visited[z-1][nx][ny]:
                    if day_night == 0:
                        temp.append((nx, ny, z-1, cnt, 1))
                    else:
                        temp.append((nx, ny, z-1, cnt+1, 1))
                    visited[z - 1][nx][ny] = True
                elif grid[nx][ny] == 0 and not visited[z][nx][ny]:
                    temp.append((nx, ny, z, cnt, day_night))
                    visited[z][nx][ny] = True
        queue = temp
    return -1


N, M, K = map(int, input().split())
grid = [list(map(int, input().strip())) for _ in range(N)]
print(bfs())
